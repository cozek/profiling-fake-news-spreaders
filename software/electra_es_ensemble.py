...
def main(indir, outdir):
    ...
    newpath = os.path.join(outdir, "es")
    ...
    args = argparse.Namespace(
        ...
        batch_size=15,
        models_dir="/home/kaushikamardas20/results_es/models",
        output_dir=newpath,
        lang="es",
    )

    print("Making Dataframe")
    es_data_df = make_fake_news_data_df(args.es_data_dir, args.lang, True)
    es_data_df["label"] = (np.random.rand(len(es_data_df)) > 0.5).astype(
        int
    )  # dummy labels

    es_data_df["split"] = "val"

    tokenizer = AutoTokenizer.from_pretrained("skimai/electra-small-spanish")

    print("Making Dataset")

    dataset = FakeNewsDataset(es_data_df, tokenizer, 512)

    # %% [code]
    saved_models = [
        os.path.join(args.models_dir, i) for i in os.listdir(args.models_dir)
    ]

    # %% [code]
    print("Loading Model")

    model = TransformerForSequenceClassification(2, "skimai/electra-small-spanish")
    ...
    for model_path in saved_models:
        ...
        print(f"Finished with Model: {model_path} ")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-m", "--modeldir", help="directory containing .pt files", required=True
    )
    ...
    # print("Copying input run")
    # for root, dirs, files in os.walk(args.input_run):
    #     for file in files:
    #         if file.endswith(".xml"):
    #             # print(os.path.join(root, file))
    #             file_name = os.path.basename(file)
    #             copyfile(os.path.join(root, file), os.path.join(en_path, file_name))
